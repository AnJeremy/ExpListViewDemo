...
public class CountDownProgress extends View {
    ...
    private static final int SMALL_CIRCLE_RADIUS = 2;
...
//        float textWidth = textPaint.measureText(textDesc);
    ...
    //    canvas.drawText(textDesc, defaultCircleRadius-textWidth/2, defaultCircleRadius-textHeight, textPaint);
        ...
        //画小圆
        float currentDegreeFlag =  (circle*currentAngle + extraDistance);
    ...
    public void setCirclePause(){
        if(animator != null){
           ...
           /* sharedPreferences = getContext().getSharedPreferences(AlarmConstants.COUNTDOWN, Activity.MODE_PRIVATE);
            SharedPreferences.Editor edit = sharedPreferences.edit();
            edit.putBoolean(AlarmConstants.IS_STOP,true);

              edit.putLong(AlarmConstants.COUNTDOWN_REMAIN,allTimerCount*1000);*/

            //sharedPreferences.edit().putLong(AlarmConstants.COUNTDOWN_REMAIN,)
           // edit.commit();
    ...
    public void setCircleResume(){
        ...
        if(animator != null){
                ...
                startTimer(etHour,etMin,etSec);

          /*  sharedPreferences = getContext().getSharedPreferences(AlarmConstants.COUNTDOWN, Activity.MODE_PRIVATE);
            sharedPreferences.edit().putBoolean(AlarmConstants.IS_STOP,false).commit();*/

            //调用倒计时操作
            //countdownMethod();
            if (totalTime != 0) {
                countdownMethodAgain();
            }else {
                countdownMethod();
            }
        }
    ...
    public void startTimer(TextView etHour, TextView etMin, TextView etSec){
        ...
        if (timerTask==null) {
            ...
            time = allTimerCount;
           /* if(time >= 10){
                AlarmUtils.startAlarmTimer(getContext(),allTimerCount*1000);
            }*/
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                case MSG_WHAT_TIME_TICK:
                    ...
                    Log.d("timer",allTimerCount+"");

                    //Log.d("timeClock",allTimerCount+"");
                    ...
                    etSec.setText(sec < 10 ? "0"+sec : sec+"" );
    ...
    private static final int MSG_WHAT_TIME_IS_UP_AGAIN = 3;
    private static final int MSG_WHAT_TIME_TICK_AGAIN = 4;
    ...
    private long time;//判断倒计时是否使用闹钟提醒 当倒计时 < 10秒 不使用闹钟提醒
    ...
    private long remainTime,totalTime; //剩余时长 ，与总时长

    private SharedPreferences sharedPreferences;//存储倒计时的状态

    private final String TAGAGAIN = "againTime";
    private final String TAG = "Time";

    //当倒计时时间没有完成 应用被强行关闭后，提供一个外界可以设置的倒计时数值
    public void setCountdownTimeAgain(long countdownTime, long totalTime, TextView etHour, TextView etMin, TextView etSec){
        this.remainTime = countdownTime;
        this.totalTime = totalTime;

        Log.d(TAGAGAIN,"remainTime"+remainTime+"totalTime"+totalTime);
    ...
    public void setCountDownTimeStop(final long remainTime, long totalTime, TextView etHour, TextView etMin, TextView etSec){
        ...
        circle = ( (float)remainTime/ totalTime )  * 360f;//剩余的弧度
        Log.d("circleDU",circle+">>"+(float) remainTime/totalTime+">>>>>"+totalTime);
        ...
        animator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                Log.d(TAGAGAIN,"计时结束____Stop"+"remainTime"+remainTime+"time"+time+"allTimerCount"+allTimerCount);
                circle = 360f;

                /*if(time<10 && allTimerCount <= 1){
                    Intent intent = new Intent(getContext(),TimerOnTimeActivity.class);
                    getContext().startActivity(intent);
                    // new AlertDialog.Builder(getContext()).setTitle("Time is up").setMessage("Time is up").setNegativeButton("Cancel", null).show();
                }*/
    ...
    public void startCountDownTimeAgain(final OnCountdownFinishListener countdownFinishListener){
        ...
        animator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                Log.d(TAGAGAIN,"计时结束____Again");
                if(countdownFinishListener != null){
                    countdownFinishListener.countdownFinished();
                    stopTimer();
                    circle = 360f;

                    Log.d(TAG,"计时结束____Stop"+"remainTime"+remainTime+"time"+time+"allTimerCount"+allTimerCount);
                    /*if(time<10 && remainTime <= 1000){
                        Intent intent = new Intent(getContext(),TimerOnTimeActivity.class);
                        getContext().startActivity(intent);
                        // new AlertDialog.Builder(getContext()).setTitle("Time is up").setMessage("Time is up").setNegativeButton("Cancel", null).show();
                    }*/
    ...
    public void countdownMethodAgain() {
        ...
        countDownTimer = new CountDownTimer(remainTime, 1000) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                Log.e(TAGAGAIN, remainTime + "timeCount>>>again");
                //刷新view
                invalidate();
    ...
    public  void setTimeAgain(){
        ...
        long hourAgain = time/60/60;
        long minAgain = (time/60)%60;
        long secAgain = time%60 ;

        etHour.setText(hourAgain < 10 ? "0"+hourAgain : hourAgain+"" );
        etMin.setText(minAgain < 10 ? "0"+minAgain : minAgain+"" );
        etSec.setText(secAgain < 10 ? "0"+secAgain : secAgain+"" );
    }
